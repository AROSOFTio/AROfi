...
guard, locks seamless same-business AP roaming, locks the simplified Agent
selling workflow, locks the operator-facing MikroTik onboarding command, then
verifies the MikroTik captive/session and no-automatic-MAC-auth policies.
...
AGENT_SALES_GUARD = ROOT / "scripts/enforce_agent_sales_redesign.py"
...
def enforce_agent_sales_redesign() -> None:
    if not AGENT_SALES_GUARD.exists():
        raise RuntimeError("Agent sales redesign guard is missing")
    runpy.run_path(str(AGENT_SALES_GUARD), run_name="__main__")
...
def main() -> None:
    ...
    print(
        ...
        "disconnect guard, seamless cross-AP roaming, simplified Agent sales, RouterOS 6/7 "
        "IP-first onboarding, captive-flow state, and no-automatic-MAC-auth policy verified."
    )
