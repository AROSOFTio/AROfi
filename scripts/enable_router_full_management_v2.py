#!/usr/bin/env python3
"""Deterministically enable full router editing and audit-safe deletion."""
...
"""
    ...
    elif "body.reason ?? ''" not in text:
        raise RuntimeError('router delete controller anchor missing')
...
""",
    ...
    replacement = r'''  async deleteRouter(routerId: string, reason: string, tenantId?: string) {
...
'''
    if 'async deleteRouter(routerId: string, reason: string' not in text:
...
def patch_routers_service() -> None:
    ...
    replacement = r'''  async updateRouter(routerId: string, dto: UpdateRouterDto, tenantId?: string) {
    ...
    if (!existing) throw new NotFoundException('Router not found')
    if (existing.tags.includes('AROFI_DELETED')) {
      throw new BadRequestException('Deleted routers cannot be edited')
    }

    if (dto.name !== undefined && !dto.name.trim()) throw new BadRequestException('Router name cannot be empty')
    if (dto.host !== undefined && !dto.host.trim()) throw new BadRequestException('Router host cannot be empty')
    if (dto.username !== undefined && !dto.username.trim()) throw new BadRequestException('RouterOS API username cannot be empty')
...
'''
    if 'Deleted routers cannot be edited' not in text:
...
def patch_overview() -> None:
    ...
    old_query = "        this.prisma.router.findMany({\n          where: tenantWhere,"
    ...
    elif "NOT: { tags: { has: 'AROFI_DELETED' } }" not in text:
        raise RuntimeError('router overview deletion filter anchor missing')

    if 'hotspotId: true' not in text:
        text = replace_once(text, "            groupId: true,\n            name: true,", "            groupId: true,\n            hotspotId: true,\n            name: true,", 'overview hotspot id')
    if 'lastScriptMode: true' not in text:
        text = replace_once(
            text,
            "            connectionMode: true,\n            siteLabel: true,",
            "            connectionMode: true,\n            username: true,\n            radiusNasIpAddress: true,\n            lastScriptMode: true,\n            siteLabel: true,",
    ...
    if 'lastScriptMode: router.lastScriptMode' not in text:
        text = replace_once(
            text,
...
def patch_client_api() -> None:
    ...
    if 'clientDeleteApi<T>(path: string, payload?: unknown)' not in text:
        text = replace_once(
            text,
...
""",
            """export async function clientDeleteApi<T>(path: string, payload?: unknown): Promise<T> {
  const doFetch = () =>
    fetch(`${browserApiBase}${path}`, {
      method: 'DELETE',
      credentials: 'include',
...
"""
    if old_delete_handler in text:
        text = text.replace(
            old_delete_handler,
            """  // Action: Delete router. Historical business records are archived, never destroyed.
  const handleDeleteRouter = async () => {
    if (!deleteModalRouter) return
...
""",
    ...
    for invariant in ('Edit Router', 'RouterFullEditModal', 'deleteReason', 'archivedHistory?: boolean'):
...
def verify() -> None:
    checks = {
        DTO: ['groupId?: string | null', 'radiusNasIpAddress?: string | null', 'scriptMode?: RouterScriptMode'],
        CONTROLLER: ["body.reason ?? ''"],
        LIFECYCLE: ["const DELETED_TAG = 'AROFI_DELETED'", "action: 'router.deleted'", 'archivedHistory'],
        ROUTERS_SERVICE: ['Deleted routers cannot be edited', 'dto.portalWalledGardenHosts', "tags: { has: 'AROFI_DELETED' }"],
        OVERVIEW: ["tags: { has: 'AROFI_DELETED' }", 'lastScriptMode: router.lastScriptMode'],
        CLIENT_API: ['clientDeleteApi<T>(path: string, payload?: unknown)'],
        PAGE: ['RouterFullEditModal', 'Reason for deletion', 'Edit Router'],
    }
    for path, markers in checks.items():
        text = require(path)
        for marker in markers:
            if marker not in text:
                raise RuntimeError(f'{path.relative_to(ROOT)} missing invariant: {marker}')
...
def main() -> None:
    ...
    patch_settings_page()
    verify()
    print('Router full management v2 enabled: onboarding details editable; deletion requires a reason and preserves historical transactions when present.')
