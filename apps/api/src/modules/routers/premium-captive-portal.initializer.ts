...
export class PremiumCaptivePortalInitializer implements OnModuleInit {
  ...
  private applyPremiumPortal(html: string) {
    ...
    // Remove the legacy captive template's decorative blue/green gradient and
    // blue accent tokens before the premium shell is injected. This keeps the
    // router-hosted login.html clean even if a browser briefly paints the base
    // markup before JavaScript finishes loading tenant context.
    html = html
      .replaceAll('linear-gradient(180deg,#f0f9ff 0%,#f0fdf4 100%)', '#f5f7f9')
      .replaceAll('#2563EB', 'var(--portal-primary)')
      .replaceAll('#1d4ed8', 'var(--portal-primary)')
      .replaceAll('#1e40af', 'var(--portal-primary)')
      .replaceAll('#bfdbfe', '#e2e8f0')
      .replaceAll('#dbeafe', '#e2e8f0')
      .replaceAll('#eff6ff', '#f8fafc')
      .replaceAll('rgba(37,99,235,.10)', 'rgba(18,32,51,.06)')
      .replaceAll('rgba(37,99,235,.12)', 'rgba(18,32,51,.06)')
      .replaceAll('rgba(29,78,216,.5)', 'rgba(18,32,51,.16)')

    const style = `<style id="arofi-premium-captive-v2">
      :root{--portal-primary:#22A53A;--ink:#122033;--muted:#64748b}
      body{background:#f5f7f9!important;color:var(--ink)!important;padding:10px 8px 22px!important}
      .card{max-width:900px!important;padding:0!important;overflow:hidden;background:#fff!important;border:0!important;border-radius:22px!important;box-shadow:0 10px 32px rgba(18,32,51,.08)!important}
      .hdr{display:none!important}
      .premium-hero{position:relative;overflow:hidden;min-height:150px;padding:14px 18px 17px;color:#fff;background:var(--portal-primary)}
      ...
      .premium-copy{position:relative;z-index:2;margin-top:15px;max-width:460px}.premium-copy h1{font-size:28px;line-height:1.02;margin:0;font-weight:900;letter-spacing:-.03em}.premium-copy p{margin:5px 0 0;color:rgba(255,255,255,.86);font-size:11.5px}.premium-copy small{display:block;margin-top:5px;color:rgba(255,255,255,.70);font-size:10.5px;font-weight:700}
      .premium-wifi{position:absolute;right:42px;bottom:24px;width:94px;height:70px;opacity:.92}.premium-wifi span{position:absolute;left:50%;bottom:8px;border:3px solid rgba(255,255,255,.72);border-left-color:transparent;border-bottom-color:transparent;border-radius:50%;transform:translateX(-50%) rotate(-45deg);animation:none}.premium-wifi span:nth-child(1){width:82px;height:82px;animation-delay:0s}.premium-wifi span:nth-child(2){width:56px;height:56px;animation-delay:.18s}.premium-wifi span:nth-child(3){width:30px;height:30px;animation-delay:.36s}.premium-wifi i{position:absolute;left:50%;bottom:2px;width:7px;height:7px;background:rgba(255,255,255,.82);border-radius:50%;transform:translateX(-50%);box-shadow:none}@keyframes arofiWave{0%,100%{opacity:.42;filter:none}50%{opacity:1;filter:none}}
      .premium-tabs{display:flex;overflow-x:auto;white-space:nowrap;border-bottom:1px solid #e2e8f0;background:#fff;padding:0 4px;scrollbar-width:none}.premium-tabs::-webkit-scrollbar{display:none}.premium-tab{position:relative;min-width:108px;flex:1;border:0;background:transparent;padding:9px 6px 8px;color:#64748b;font-size:9.5px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px}.premium-tab svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2}.premium-tab.on{color:var(--portal-primary)}.premium-tab.on:after{content:"";position:absolute;left:20%;right:20%;height:2.5px;bottom:0;border-radius:5px;background:var(--portal-primary)}
      #loading{padding:22px 0!important;background:#f5f7f9}.spin-wrap .spinner{border-color:#bfdbfe!important;border-top-color:var(--portal-primary)!important}
      #content{background:#f5f7f9;padding:10px!important}.premium-panel{display:none}.premium-panel.on{display:block}.premium-card{background:#fff;border:1px solid #e2e8f0;border-radius:17px;padding:12px;box-shadow:0 4px 14px rgba(7,26,73,.035);margin-bottom:9px}.premium-card-title{display:flex;align-items:center;gap:6px;font-size:14px;font-weight:900;color:var(--ink);margin-bottom:9px}.premium-card-title i{font-style:normal;color:var(--portal-primary)}
      .quick-row{margin:0!important;display:grid!important;grid-template-columns:1fr auto!important;gap:7px!important}.quick-row input{border:1px solid #d9e2ef!important;border-radius:11px!important;background:#fff!important;padding:10px 11px!important;font-size:12px!important}.connect-btn,.btn{border-radius:11px!important;background:var(--portal-primary)!important;padding:10px 15px!important;box-shadow:none!important}.secure-note{display:flex;align-items:center;gap:5px;margin-top:8px;font-size:9.5px;color:#64748b;font-weight:650}
      .section-label{text-align:left!important;font-size:14px!important;font-weight:900!important;color:var(--ink)!important;margin:3px 0 2px!important}.section-sub{text-align:left!important;margin:0 0 9px!important;max-width:none!important;font-size:10.5px!important}.pkgs{grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:8px!important;margin-top:8px!important}.pkg{display:flex!important;min-width:0!important;min-height:116px!important;flex-direction:column!important;align-items:stretch!important;gap:4px!important;padding:10px!important;border-radius:14px!important;border:1px solid #dfe7f2!important;box-shadow:0 3px 10px rgba(7,26,73,.035)!important}.pkg .pk-name{font-size:12.5px!important;line-height:1.18!important;color:var(--ink)!important;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.pkg .pk-dur{font-size:9.5px!important;line-height:1.2!important}.pkg .pk-price{font-size:15px!important;line-height:1.05!important;color:var(--portal-primary)!important;margin:1px 0 3px}.pkg .pk-buy{margin-top:auto!important;text-align:center;border:0!important;border-radius:8px!important;background:var(--portal-primary)!important;padding:6px!important;font-size:9.5px!important}.pkg.sel{border-color:var(--portal-primary)!important;box-shadow:0 0 0 1px var(--portal-primary)!important}
      ...
      .message-box.info{border-color:#e2e8f0!important;background:#f8fafc!important;color:var(--portal-primary)!important}.tv-pay-note{border-color:#e2e8f0!important;background:#f8fafc!important;color:var(--portal-primary)!important}.find-wrap{display:none!important}.find-panel{display:block!important;margin:0!important;border:0!important;padding:0!important}.tv-voucher{margin:0 0 9px!important;border:1px solid #e2e8f0!important;background:#fff!important;border-radius:14px!important}.tv-section{display:block;margin:0!important;border:0!important;background:transparent!important;padding:0!important}.tv-section .section-label{margin-top:0!important}.tv-section .pkgs{margin-top:8px!important}#multiSection{display:block!important}
      ...
      .support{display:none;margin:9px 0 0!important;border:1px solid #e2e8f0!important;border-radius:17px!important;background:#fff!important;padding:12px!important}.support[style*="flex"]{display:flex!important}.support-phone{color:var(--portal-primary)!important}.tech{margin:9px auto 0!important;color:#64748b!important}.wa-inline{box-shadow:none!important}.pay-box{border-radius:17px!important}.message-box{border-radius:16px!important}
    ...
    const hero = `<div class="premium-hero">
      <div class="premium-brand"><img id="premiumLogo" class="premium-logo" src="https://arofi.net/brand-assets/arofi-logo" alt="AROFi"><div class="premium-lang">◎ English⌄</div></div>
