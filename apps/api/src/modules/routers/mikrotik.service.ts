...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    return [
      ...
      `/ip firewall filter add chain=forward action=accept src-address=${subnet} comment="AROFi hotspot forward" place-before=0`,
      `/ip firewall filter add chain=forward action=accept dst-address=${subnet} connection-state=established,related comment="AROFi hotspot forward" place-before=0`,
      ``,
