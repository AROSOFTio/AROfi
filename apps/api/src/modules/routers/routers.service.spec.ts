...
describe('RoutersService', () => {
  ...
  describe('remote access allocation', () => {
    ...
    it('generates a RouterOS 6-compatible SSTP client script that cannot become a default route', async () => {
      ...
      expect(script).toContain(':local sstpTarget "arofi.net:4443"')
      expect(script).toContain('connect-to=$sstpTarget')
      expect(script).toContain(':global arofiRemoteAccessStatus "failed"')
      expect(script).toContain(':set arofiRemoteAccessStatus "ok"')
      expect(script).toContain('SSTP client could not be enabled or verified')
      expect(script).toContain('AROFi Remote Access configured; tunnel pending connection.')
      expect(script).not.toContain(' port=4443')
