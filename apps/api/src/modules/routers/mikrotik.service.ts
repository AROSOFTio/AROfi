...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    const hotspotProfile = [
      ...
      `/ip hotspot profile set [find name="${profileName}"] use-radius=yes radius-accounting=yes radius-interim-update=1m html-directory=hotspot login-by=mac,cookie,http-pap split-user-domain=no radius-location-id="${this.escape(registrationKey)}" radius-location-name="${this.escape(registrationKey)}"${input.dnsName ? ` dns-name="${this.escape(input.dnsName)}"` : ''}`,
