...
export function buildRemoteAccessInstallCommand(remoteToken: string | null | undefined, origin?: string) {
  ...
  return `${DNS_BOOTSTRAP}:global arofiRemoteAccessStatus "not-run"; :global arofiRemoteAccessMessage ""; :do { /file remove [find name="vpn.rsc"] } on-error={}; /tool fetch url="${apiOrigin}/mikrotik/remote-access/install/${remoteToken || ''}" check-certificate=no dst-path="vpn.rsc" mode=https; :delay 4s; :local f [/file find name="vpn.rsc"]; :if ([:len $f] > 0) do={ :local sz [/file get $f size]; :if ($sz > 0) do={ :put "AROFi remote access downloaded. Installing..."; :delay 2s; /import file-name="vpn.rsc"; :delay 1s; /file remove "vpn.rsc"; :global arofiRemoteAccessStatus; :global arofiRemoteAccessMessage; :if ($arofiRemoteAccessStatus = "ok") do={ :put "AROFi remote access installed." } else={ :put ("ERROR: AROFi remote access was not installed. " . $arofiRemoteAccessMessage); :put "Fix the error above, then re-run this command." } } else={ :put "ERROR: remote access file is empty. Re-paste when WAN is stable."; /file remove $f } } else={ :put "ERROR: remote access file was not downloaded. Re-paste when WAN is stable." }`
}
