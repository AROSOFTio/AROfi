...
export function buildRemoteAccessInstallCommand(remoteToken: string | null | undefined, origin?: string) {
  const host = origin || (typeof window !== 'undefined' ? window.location.origin : 'https://arofi.arosoftlabs.com')
...
export function buildSetupFallbackCommand(registrationKey: string) {
  return `${DNS_BOOTSTRAP}/tool fetch url="https://arofi.arosoftlabs.com/api/mikrotik/script/${registrationKey}" dst-path="arofi-setup.rsc" mode=https; /import file-name="arofi-setup.rsc"; /file remove "arofi-setup.rsc"`
}
