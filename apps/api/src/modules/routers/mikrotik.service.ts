...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    return [
      ...
      `:local arofiLanBridge "bridge"`,
      `:if ([:len [/interface bridge find name="bridgeLocal"]] > 0) do={`,
      `  :set arofiLanBridge "bridgeLocal"`,
      `}`,
      ...
      `  }`,
      `  :foreach staleWanClient in=[/ip dhcp-client find interface=$arofiLanBridge] do={`,
      `    /ip dhcp-client remove $staleWanClient`,
      `  }`,
      ...
      `  }`,
      `}`,
      `:if ([:len [/interface bridge find name=$arofiLanBridge]] = 0) do={`,
      `  /interface bridge add name=$arofiLanBridge comment="AROFi LAN bridge"`,
      `}`,
      ...
      `    /interface bridge port add bridge=$arofiLanBridge interface=$pId`,
      `  }`,
      `}`,
      ``,
      `# Fresh captive Wi-Fi setup. Supports RouterOS v7 /interface wifi and legacy /interface wireless.`,
      ...this.buildOpenWifiScript(ssid, '$arofiLanBridge'),
      ``,
      ...
      `  /ip address add address=10.50.0.1/24 interface=$arofiLanBridge`,
      `}`,
      ...
      `}`,
      `:if ([:len [/ip dhcp-server find interface=$arofiLanBridge]] = 0) do={`,
      `  /ip dhcp-server add name=arofi-dhcp interface=$arofiLanBridge address-pool=arofi-pool disabled=no`,
      `} else={`,
      `  /ip dhcp-server set [find interface=$arofiLanBridge] address-pool=arofi-pool disabled=no`,
      `}`,
      ...
      `:if ([:len [/ip hotspot find name="${this.escape(hotspotName)}"]] = 0) do={`,
      `  /ip hotspot add name="${this.escape(hotspotName)}" interface=$arofiLanBridge address-pool=arofi-pool profile="${profileName}" disabled=no`,
      `}`,
      ``,
      `# 6. Tell AROFi this script was imported. This lets AROFi learn the router public/NAT IP for RADIUS.`,
      ...callbackScript,
