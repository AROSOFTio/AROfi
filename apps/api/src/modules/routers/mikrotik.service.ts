...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    const antiTether = [
      ``,
      `# TTL anti-tethering: two-stage enforcement`,
      `# Stage 1 (mangle): decrement TTL by 1 on every packet entering the hotspot bridge.`,
      `#   Direct device starts at TTL=64 → exits router at 63 → normal internet traffic.`,
      `#   Tethered phone creates a NAT hop: laptop TTL=64 → phone NAT: 63 → router mangle: 62.`,
      `# Stage 2 (filter): DROP any forward packet whose TTL is <= 1 after decrements.`,
      `#   A tethered device starting at TTL=1 or TTL=2 is killed before it exits.`,
      `#   Devices that entered with TTL=64 are safe (62 >> 1).`,
      `# Together these stop phone-hotspot-behind-wifi-hotspot sharing.`,
      ...
      `    :do { /ip firewall filter add chain=forward ttl=less-than:2 in-interface=$hotInterface action=drop comment="AROFi anti-tether-drop" } on-error={}`,
      `  }`,
      `}`,
      `# Move tether-drop rules before any accept rules so they take effect`,
      `:foreach r in=[/ip firewall filter find comment="AROFi anti-tether-drop"] do={`,
