...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    const safeScript = [
      ...
      ``,
      `# 1. Identity and RouterOS API/WinBox services`,
      ...
      ...(process.env.MIKROTIK_SETUP_ADMIN_PASSWORD
        ? [
            `:do { /user set [find name="admin"] password="${this.escape(process.env.MIKROTIK_SETUP_ADMIN_PASSWORD)}" } on-error={ :put "Warning: could not set admin password." }`,
          ]
        : []),
      ``,
    ...
    return [
      ...
      `# Safe mode note: AROFi will not remove ether1 from bridge here because that can disconnect WinBox during setup.`,
      `:if ([:len [/interface bridge find name="bridgeLocal"]] = 0) do={`,
      `  /interface bridge add name=bridgeLocal comment="AROFi LAN bridge"`,
      `}`,
      ...
      `    /interface bridge port add bridge=bridgeLocal interface=$pId`,
      `  }`,
      `}`,
      ``,
      `# Fresh captive Wi-Fi setup. Supports RouterOS v7 /interface wifi and legacy /interface wireless.`,
      ...this.buildOpenWifiScript(ssid, 'bridgeLocal'),
      ``,
      ...
      `}`,
      `:if ([:len [/ip dhcp-server find interface=bridgeLocal]] = 0) do={`,
      `  /ip dhcp-server add name=arofi-dhcp interface=bridgeLocal address-pool=arofi-pool disabled=no`,
      `} else={`,
      `  /ip dhcp-server set [find interface=bridgeLocal] address-pool=arofi-pool disabled=no`,
      `}`,
      ...
      `}`,
      `:put "AROFi HotSpot/Wi-Fi section complete. If internet does not pass to clients, run the WAN split commands from the Router page after reconnecting through ether2/LAN or the new Wi-Fi."`,
      ``,
      `# 6. Tell AROFi this script was imported. This lets AROFi learn the router public/NAT IP for RADIUS.`,
      ...callbackScript,
      ``,
  ...
  private buildOpenWifiScript(ssid: string, bridgeName = 'bridge') {
    ...
    return [
      ...
      `    /interface wireless set [find name="wlan1"] disabled=no mode=ap-bridge ssid=$arofiSsid security-profile=arofi-open`,
      `    :if ([:len [/interface bridge port find interface="wlan1"]] = 0) do={`,
      `      /interface bridge port add bridge=${bridgeName} interface=wlan1`,
      `    }`,
      `  }`,
