...
// ITU-assigned country calling codes. Uganda is listed first since it's the
// default selection for registration; the rest are alphabetical by name.
...
export function findCountryByIso2(iso2: string): CountryCode {
  return COUNTRY_CODES.find((c) => c.iso2 === iso2) ?? DEFAULT_COUNTRY
}

// Strips a leading trunk "0" (common in local number formats, e.g. Uganda's
// 0771234567) before the number is prefixed with the country's dial code.
export function stripLeadingTrunkZero(localNumber: string): string {
  const digits = localNumber.replace(/\D/g, '')
  return digits.startsWith('0') ? digits.slice(1) : digits
}

export function toE164(country: CountryCode, localNumber: string): string {
  return `+${country.dialCode}${stripLeadingTrunkZero(localNumber)}`
}

// Loose sanity check — national numbers (excluding the trunk 0) are almost
// always 6-12 digits worldwide. Exact per-country rules aren't worth the
// complexity here; this just catches empty/garbage input at signup.
export function isPlausibleNationalNumber(localNumber: string): boolean {
  const digits = stripLeadingTrunkZero(localNumber)
