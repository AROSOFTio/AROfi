...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    const safeScript = [
      ...
      `/radius add service=hotspot address=${input.radiusHost} \\`,
      `  secret="${this.escape(input.sharedSecret)}" \\`,
      `  authentication-port=${input.radiusAuthPort} \\`,
      `  accounting-port=${input.radiusAccountingPort} \\`,
      `  timeout=3s comment="AROFi ${this.escape(registrationKey)}"`,
      ``,
      ...
      `/ip hotspot profile set [find name="${profileName}"] \\`,
      `  radius-location-name="${this.escape(registrationKey)}"`,
      `/ip hotspot profile set [find name="${profileName}"] \\`,
      `  radius-location-id="${this.escape(registrationKey)}"`,
      ...
      `  /ip hotspot set [find name="${this.escape(hotspotName)}"] \\`,
      `    profile="${profileName}"`,
      `} else={`,
    ...
    return [
      ...
      `      action=masquerade comment="AROFi nat"`,
      `  }`,
      `}`,
      `:if ([:len [/ip hotspot find name="${this.escape(hotspotName)}"]] = 0) do={`,
  ...
  private buildWalledGarden(hosts: string[]) {
    ...
    return [
      ...
      ...normalizedHosts.map(
        (host) =>
          `/ip hotspot walled-garden add dst-host="${this.escape(host)}" \\
  action=allow comment="AROFi portal"`,
      ),
    ]
  }
