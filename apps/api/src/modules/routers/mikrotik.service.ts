...
export class MikrotikService {
  ...
  buildProvisioningScript(input: ProvisioningInput) {
    ...
    return [
      ...
      `      :set wanIface [/ip dhcp-client get ($boundDhcp->0) interface]`,
      `    }`,
      `  } on-error={}`,
      `}`,
      `:if ($wanIface = "") do={`,
      ...
      `      :set wanIface [/interface pppoe-client get ($activePppoe->0) name]`,
      `    }`,
      `  } on-error={}`,
      `}`,
      `:if ($wanIface = "") do={`,
      ...
      `      :set wanIface [/interface lte get ($activeLte->0) name]`,
      `    }`,
      `  } on-error={}`,
      `}`,
  ...
  private buildProvisioningCallbackScript(callbackUrl: string, fallbackCallbackUrl: string) {
    return [
      ...
      `    :local boundDhcp [/ip dhcp-client find status=bound]`,
      `    :if ([:len $boundDhcp] > 0) do={`,
      `      :set cbWanIface [/ip dhcp-client get ($boundDhcp->0) interface]`,
      `    }`,
      `  } on-error={}`,
      `}`,
      `:if ($cbWanIface = "") do={`,
      ...
      `    :local activePppoe [/interface pppoe-client find running=yes]`,
      `    :if ([:len $activePppoe] > 0) do={`,
      `      :set cbWanIface [/interface pppoe-client get ($activePppoe->0) name]`,
      `    }`,
      `  } on-error={}`,
      `}`,
      `:if ($cbWanIface = "") do={`,
      ...
      `    :local activeLte [/interface lte find running=yes]`,
      `    :if ([:len $activeLte] > 0) do={`,
      `      :set cbWanIface [/interface lte get ($activeLte->0) name]`,
      `    }`,
      `  } on-error={}`,
      `}`,
      `:do {`,
