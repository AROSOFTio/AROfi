  ...
  private buildLoginHtmlInstallScript(
    ...
    const profileSet = profileName
      ? [
          ...
          `  /ip hotspot profile set [find name="${this.escape(profileName)}"] html-directory=$arofiPortalDir`,
    ...
    return [
      ...
      `:do {`,
      `  /tool fetch url="${loginHtmlUrl}" check-certificate=no mode=https dst-path=$arofiLoginPath`,
      `  :if ([:len [/file find name=$arofiLoginPath]] > 0) do={`,
      ...
      `} on-error={`,
      `  :do {`,
      `    /tool fetch url="${fallbackLoginHtmlUrl}" mode=http dst-path=$arofiLoginPath`,
      `    :if ([:len [/file find name=$arofiLoginPath]] > 0) do={`,
      ...
      `    :put "Fix: /tool fetch url=\\"${loginHtmlUrl}\\" dst-path=\\"$arofiLoginPath\\""`,
      `  }`,
      `}`,
      ...
      `:do {`,
      `  /tool fetch url="${statusHtmlUrl}" check-certificate=no mode=https dst-path=$arofiStatusPath`,
      `  :if ([:len [/file find name=$arofiStatusPath]] > 0) do={`,
      ...
      `} on-error={`,
      `  :do {`,
      `    /tool fetch url="${fallbackStatusHtmlUrl}" mode=http dst-path=$arofiStatusPath`,
      `    :if ([:len [/file find name=$arofiStatusPath]] > 0) do={`,
